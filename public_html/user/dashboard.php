...
<head>
    ...
    <title data-lang="dashboard.title"><?php echo Lang::t('dashboard.title'); ?></title>
...
<main class="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 md:mb-6 gap-2">
        <h3 class="text-xl md:text-2xl font-bold text-white">
            <span data-lang="dashboard.welcome"><?php echo Lang::t('dashboard.welcome'); ?></span> <?php echo htmlspecialchars($user['username']); ?> ⭐
        </h3>

        <span class="px-2 py-1 rounded-full text-xs md:text-sm font-medium 
        <?php echo $stats['status'] == 'active' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'; ?>"
        data-lang="<?php echo $stats['status'] == 'active' ? 'dashboard.status.active' : 'dashboard.status.banned'; ?>">
