...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    if (!$clientUsesProductRules) {
        storeBridgeClientProductRedirect($clientId, '', $t('โหมดหักเงินจากบัญชีตัวแทนจะใช้รายการสินค้า LOCAL ที่ Store API เปิดให้และราคาตัวแทนจริงของบัญชีที่ผูก จึงไม่รับกฎสินค้า/ราคาพิเศษของ API แบบเดิม', 'Linked reseller-wallet clients use the Store API provider\'s active LOCAL catalogue and the linked account effective reseller price, so legacy per-product API visibility/custom-price rules are disabled.'), $query);
    }
    if ($action === 'save_rule') {
...
<div class="rounded-xl border border-amber-500/30 bg-amber-950/20 p-5 text-sm text-amber-100">
    ...
    <div class="mt-2 text-amber-100/80"><?php echo $h($t('API Key นี้จะเห็นสินค้า/Variant LOCAL ที่ Store API เปิดใช้งานและมีสถานะ Active พร้อมใช้ราคาตัวแทนจริงของบัญชีที่ผูก (รวมราคาพิเศษรายบัญชี) โดยไม่ใช้ Custom API Price, Multiplier หรือสถานะปิดสินค้าเก่าจาก store_api_client_products ทั้งนี้ Provider Endpoint นี้ยังส่งออกเฉพาะสต็อกคีย์ LOCAL ไม่ได้ดึงสต็อก Supplier/CGO มาขายต่ออัตโนมัติ', 'This API key sees active LOCAL products/variants exposed by the Store API and uses the linked account\'s effective reseller price (including account-specific pricing). Legacy Custom API Price, multiplier, and disabled-product rules are ignored. This provider endpoint still exports local key stock only; it does not automatically proxy supplier/CGO-only stock.')); ?></div>
</div>
<?php else: ?>
