...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && ($_POST['action'] ?? '') === 'provider_probe') {
    ...
    try {
        $debug = trueMoneyRunSafeProviderProbe($adminId);
...
$providerTarget = trueMoneyDebugProviderUrlSummary((string) TM_API_URL);
$documentedTarget = trueMoneyDebugUrlSummary((string) ($contract['endpoint'] ?? ''));
$providerName = trueMoneyProviderName();
$endpointMatches = ($providerTarget['scheme'] ?? '') === ($documentedTarget['scheme'] ?? '') && ($providerTarget['host'] ?? '') === ($documentedTarget['host'] ?? '');
...
<main class="mx-auto max-w-7xl space-y-5 p-4 md:p-6">
    <section>
        ...
        <p class="mt-2 max-w-4xl text-sm text-gray-400"><?php echo $e($t('ตรวจ Contract ของ API, เส้นทาง DNS/TCP/TLS/HTTP, JSON response และหลักฐานที่ Provider รายงาน โดยแยกสิ่งที่ยืนยันได้ออกจากสิ่งที่ยังมองไม่เห็นฝั่ง Provider → TrueMoney', 'Inspect the API contract, DNS/TCP/TLS/HTTP path, JSON response, and provider-reported evidence while separating confirmed facts from the unseen Provider → TrueMoney hop.')); ?></p>
    </section>

    <section class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <div class="glass rounded-xl p-4"><div class="text-xs text-gray-500"><?php echo $e($t('ระบบ TrueMoney', 'TrueMoney feature')); ?></div><div class="mt-2 text-lg font-bold <?php echo getSetting('truemoney_enabled') === '1' ? 'text-green-300' : 'text-amber-300'; ?>"><?php echo $e(getSetting('truemoney_enabled') === '1' ? $t('เปิดใช้งาน', 'Enabled') : $t('ปิดใช้งาน', 'Disabled')); ?></div></div>
        <div class="glass rounded-xl p-4"><div class="text-xs text-gray-500"><?php echo $e($t('เบอร์ผู้รับ / Local validation', 'Receiver phone / local validation')); ?></div><div class="mt-2 text-lg font-bold <?php echo !empty($phoneCheck['exact_10_digits']) ? 'text-green-300' : 'text-red-300'; ?>"><?php echo $e($phoneCheck['masked'] ?: '-'); ?></div><div class="mt-1 text-xs text-gray-500">Local rule · 10 digits + starts with 0: <?php echo !empty($phoneCheck['exact_10_digits']) ? 'YES' : 'NO'; ?></div></div>
        <div class="glass rounded-xl p-4"><div class="text-xs text-gray-500"><?php echo $e($t('Provider / Endpoint', 'Provider / Endpoint')); ?></div><div class="mt-2 text-lg font-bold text-cyan-200"><?php echo $e(trueMoneyProviderDisplayName()); ?></div><div class="mono-wrap mt-1 text-xs text-white"><?php echo $e((string) ($contract['endpoint_template'] ?? $contract['endpoint'] ?? TM_API_URL)); ?></div><div class="mt-1 text-xs <?php echo $endpointMatches ? 'text-green-300' : 'text-amber-300'; ?>"><?php echo $e($endpointMatches ? $t('Host ตรงกับเอกสาร', 'Host matches documentation') : $t('Host ต่างจากเอกสาร', 'Host differs from documentation')); ?></div></div>
        <div class="glass rounded-xl p-4"><div class="text-xs text-gray-500"><?php echo $e($t('Debug Storage', 'Debug storage')); ?></div><div class="mt-2 text-lg font-bold <?php echo $debugReady ? 'text-green-300' : 'text-red-300'; ?>"><?php echo $e($debugReady ? $t('พร้อม', 'Ready') : $t('ไม่พร้อม', 'Unavailable')); ?></div><div class="mt-1 text-xs text-gray-500"><?php echo number_format(count($debugRows)); ?> recent rows</div></div>
    </section>

    <section class="glass rounded-xl border border-orange-400/20 p-4">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div class="max-w-3xl">
                <h2 class="font-bold text-white"><i class="bi bi-activity mr-2 text-orange-300"></i><?php echo $e($t('ทดสอบ HTTP Provider แบบปลอดภัย', 'Safe Provider HTTP Probe')); ?></h2>
                <p class="mt-2 text-sm text-gray-400"><?php echo $e($t('ยิง HTTP ผ่าน Provider Adapter และโค้ด redeemAngpao() ตัวเดียวกับ production โดย xpluem จะใช้ GET + token/phone ใน path ตามเอกสาร ใช้ token ปลอมแบบสุ่ม และไม่สร้าง redemption, ไม่เครดิตเงิน หรือเขียน Wallet Ledger', 'Send HTTP through the same production provider adapter and redeemAngpao() client. xpluem uses documented GET path parameters with a random synthetic token; no redemption or wallet mutation is performed.')); ?></p>
                <div class="mt-3 rounded-lg border border-white/10 bg-black/25 p-3 text-xs text-gray-400">
                    <div><strong class="text-gray-200">Active provider:</strong> <?php echo $e(trueMoneyProviderDisplayName()); ?></div>
                    <div class="mt-1"><strong class="text-gray-200">Expected useful result:</strong> สำหรับ xpluem การตอบว่า “ลิงค์ซองของขวัญไม่ถูกต้อง” หรือ error ตามเอกสารถือว่า HTTP/Application path ทำงาน โดย Probe จะไม่ใช้ซองจริง</div>
                    <div class="mt-1"><strong class="text-gray-200">Privacy:</strong> token และเบอร์อยู่ใน URL path ตาม API contract แต่ Debug จะเก็บเฉพาะ fingerprint/ความยาว/masked phone และไม่เก็บ effective URL ดิบ</div>
                </div>
            </div>
            <button id="runProbe" type="button" class="shrink-0 rounded-lg bg-orange-500 px-5 py-3 font-bold text-white hover:bg-orange-600 disabled:cursor-not-allowed disabled:opacity-50"><i class="bi bi-send-check mr-2"></i><?php echo $e($t('ทดสอบ HTTP ตอนนี้', 'Run HTTP test')); ?></button>
        </div>
    ...
    <section class="glass rounded-xl p-4">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div><h2 class="font-bold text-white"><i class="bi bi-filetype-json mr-2 text-cyan-300"></i><?php echo $e($t('Debug JSON ล่าสุด', 'Recent Debug JSON')); ?></h2><p class="mt-1 text-xs text-gray-500"><?php echo $e($t('รวมทั้ง redemption จริงและ provider probe สูงสุด 50 รายการล่าสุด', 'Latest 50 redemption and provider-probe diagnostics.')); ?></p></div>
        ...
        <div class="mt-4 overflow-x-auto">
            <table class="min-w-full text-left text-sm">
                <thead class="border-b border-white/10 text-xs text-gray-500"><tr><th class="p-3">Time</th><th class="p-3">Request</th><th class="p-3">Result</th><th class="p-3">HTTP</th><th class="p-3">Duration</th><th class="p-3">JSON</th></tr></thead>
                <tbody>
                <?php if (!$debugRows): ?>
                    <tr><td colspan="6" class="p-6 text-center text-gray-500"><?php echo $e($t('ยังไม่มี Debug JSON', 'No debug JSON yet')); ?></td></tr>
                <?php else: ?>
    ...
    <section class="glass rounded-xl p-4">
        <h2 class="font-bold text-white"><i class="bi bi-journal-code mr-2 text-violet-300"></i><?php echo $e($t('API Contract ที่ใช้ตรวจ', 'API contract used by diagnostics')); ?></h2>
        <pre class="mono-wrap mt-3 max-h-96 overflow-auto whitespace-pre-wrap rounded-lg bg-black/35 p-4 text-xs text-gray-300"><?php echo $e(json_encode($contract, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)); ?></pre>
    </section>
</main>
...
(() => {
    ...
    runButton?.addEventListener('click', async () => {
        ...
        currentProbeJson = '';
        statusBox.classList.remove('hidden'); statusBox.textContent = 'กำลังยิง HTTP จาก SAKAZUKI Server...';
        ...
        try {
            ...
            const assessment = parsed && parsed.probe && parsed.probe.assessment ? parsed.probe.assessment : null;
            const code = parsed && parsed.result ? parsed.result.code : null;
            statusBox.textContent = 'HTTP test เสร็จแล้ว' + (assessment && assessment.status ? ' · ' + assessment.status : '') + (code ? ' · ' + code : '');
    ...
    downloadProbe?.addEventListener('click', () => { if (currentProbeJson) downloadJson(currentProbeJson, 'truemoney-provider-probe-' + new Date().toISOString().replace(/[:.]/g,'-') + '.json'); });
