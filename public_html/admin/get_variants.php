...
$availableKeys = getAvailableKeys((int)$productId);
$grouped = [];
foreach ($availableKeys as $key) {
    $duration = (string)($key['duration'] ?? 'Standard');
    $price = max(0.0, (float)($key['price_reseller'] ?? 0));
    if (!isset($grouped[$duration])) {
        $grouped[$duration] = [
            'duration' => $duration,
            'price' => $price,
            'count' => 0,
...
<?php else: ?>
    <div class="space-y-2">
        ...
        ?>
            <button type="button"
                    onclick="selectVariant(<?php echo variantJsLiteral($duration); ?>, <?php echo (int)$productId; ?>, <?php echo variantJsLiteral($productName); ?>, <?php echo $count; ?>, <?php echo json_encode($price); ?>)"
