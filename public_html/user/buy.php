...
$currentStoreRole = isReseller() ? 'reseller' : 'user';
$accountSpecialPriceMap = getResellerVariantPriceMap($currentAccountId);
...
<body class="bg-gray-900 text-gray-100 min-h-screen">
            ...
            <div class="glass rounded-lg p-2 mb-4">
                <div class="marquee-container" data-announcement-marquee data-marquee-speed="52">
                    <div class="marquee-content text-sm" data-announcement-track style="--announcement-text-color: <?php echo htmlspecialchars($announcementTextColor, ENT_QUOTES, 'UTF-8'); ?>; color: <?php echo htmlspecialchars($announcementTextColor, ENT_QUOTES, 'UTF-8'); ?>;">
                        ...
                        <?php 
                            if (trim($announcementText) === '🎖️ยินดีต้อนรับเข้าสู่ร้าน SPWZ อย่างเป็นทางการ 🎖️') {
                                echo Lang::t('common.default_announcement');
                            } else {
        ...
        <div class="mb-6 space-y-4">
                ...
                <form method="GET" action="" class="flex flex-col sm:flex-row gap-2">
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="search" value="<?php echo htmlspecialchars($searchQuery, ENT_QUOTES, 'UTF-8'); ?>"
                            ...
                            class="w-full pl-9 pr-4 py-2 bg-transparent border border-white/20 rounded text-white placeholder-gray-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none text-sm">
                    </div>
                    ...
                    <?php endif; ?>
                    <button type="submit" class="px-3 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded text-sm flex items-center justify-center gap-1">
            ...
            <div class="glass rounded-lg p-3">
                <div class="flex items-center gap-2 mb-2">
                    <i class="bi bi-phone text-blue-400 text-xs"></i>
                    ...
                    <div class="flex gap-1.5 min-w-max">
                        ...
                        ?>
                            <a href="<?php echo htmlspecialchars($buildStoreUrl(['platform' => $platformKey, 'category' => 'all']), ENT_QUOTES, 'UTF-8'); ?>"
                                class="px-3 py-2 rounded-lg text-xs font-bold transition whitespace-nowrap border border-white/20 <?php echo $selectedPlatform === $platformKey ? 'bg-blue-500 text-white border-blue-400/50' : 'bg-white/5 text-gray-300 hover:bg-white/10'; ?>">
            ...
            <?php if (!empty($categories)): ?>
                <div class="glass rounded-lg p-3">
                    <div class="flex items-center gap-2 mb-2">
                        <i class="bi bi-filter text-blue-400 text-xs"></i>
                    ...
                    <div class="category-filter overflow-x-auto pb-1">
                        <div class="flex gap-1.5 min-w-max">
                            <a href="<?php echo htmlspecialchars($buildStoreUrl(['category' => 'all']), ENT_QUOTES, 'UTF-8'); ?>"
                                class="px-3 py-2 rounded-lg text-xs font-bold transition whitespace-nowrap border border-white/20 <?php echo $selectedCategory === 'all' ? 'bg-blue-500 text-white border-blue-400/50' : 'bg-white/5 text-gray-300 hover:bg-white/10'; ?>" data-lang="buy.category_all">
                            ...
                            <?php foreach ($categories as $category): ?>
                                <a href="<?php echo htmlspecialchars($buildStoreUrl(['category' => $category]), ENT_QUOTES, 'UTF-8'); ?>"
                                    class="px-3 py-2 rounded-lg text-xs font-bold transition whitespace-nowrap border border-white/20 <?php echo $selectedCategory === $category ? 'bg-blue-500 text-white border-blue-400/50' : 'bg-white/5 text-gray-300 hover:bg-white/10'; ?>">
        ...
        <div class="mb-8">
            <?php if (empty($filteredProducts)): ?>
                <div class="glass rounded-lg p-10 text-center">
                    ...
                    <?php if ($searchQuery !== '' || $selectedCategory !== 'all' || $selectedPlatform !== 'all'): ?>
                        <a href="<?php echo htmlspecialchars($buildStoreUrl(['platform' => 'all', 'category' => 'all', 'search' => '']), ENT_QUOTES, 'UTF-8'); ?>"
                            class="inline-block mt-3 px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded text-sm transition" data-lang="buy.clear_filters_btn">
                            <?php echo Lang::t('buy.clear_filters_btn'); ?>
                        </a>
                    <?php endif; ?>
                </div>
            <?php else: ?>
                <div class="flex flex-col gap-4">
                    <?php foreach ($filteredProducts as $product): ?>
                        ...
                        // When at least one local key exists for a variant, the API source is hidden.
                        $variants = $storefrontVariantMap[(int) $product['id']] ?? [];

                        // Enforce the account-specific price on the storefront even when
                        // the unified catalogue combines local stock with API stock.
                        foreach ($variants as &$variantPriceRow) {
                            $variantPriceId = (int) ($variantPriceRow['variant_id'] ?? 0);
                            $regularDisplayedPrice = round((float) ($variantPriceRow['price'] ?? 0), 2);
                            $variantPriceRow['regular_price'] = $regularDisplayedPrice;
                            $variantPriceRow['has_special_price'] = false;
                            if ($variantPriceId > 0 && array_key_exists($variantPriceId, $accountSpecialPriceMap)) {
                                $specialDisplayedPrice = round((float) $accountSpecialPriceMap[$variantPriceId], 2);
                                if (is_finite($specialDisplayedPrice) && $specialDisplayedPrice >= 0 && $specialDisplayedPrice <= 10000000) {
                                    $variantPriceRow['price'] = $specialDisplayedPrice;
                                    $variantPriceRow['has_special_price'] = abs($specialDisplayedPrice - $regularDisplayedPrice) >= 0.005;
                                }
                            }
                        }
                        unset($variantPriceRow);
                        ...
                        <div class="product-item bg-[#1b2336] rounded-[14px] border border-white/5 p-4 flex flex-col gap-4 shadow-sm">
                            ...
                            <div class="flex flex-col gap-2 mt-2">
                                <?php if (empty($variants)): ?>
                                    <div
                                        class="bg-[#242b45] border border-white/5 rounded-[10px] p-3 text-center text-gray-500 text-sm font-medium" data-lang="common.sold_out">
                                        <?php echo Lang::t('common.sold_out'); ?>
                                    </div>
                                <?php else: ?>
                                        ...
                                        <button type="button"
                                            ...
                                            <span data-inventory-normal <?php echo $isAvailable ? '' : 'hidden'; ?> class="flex justify-between items-center w-full">
                                                ...
                                                <span class="text-right">
                                                    <?php if (!empty($vData['has_special_price'])): ?>
                                                        <span class="block text-[10px] text-gray-500 line-through leading-none mb-1"><?php echo formatCurrency((float) ($vData['regular_price'] ?? 0)); ?></span>
                                                        <span class="block text-emerald-400 font-bold text-[13px]"><?php echo formatCurrency((float) $vData['price']); ?></span>
                                                        <span class="block text-[9px] text-emerald-300 mt-0.5"><?php echo getAppLang() === 'th' ? 'ราคาพิเศษเฉพาะบัญชี' : 'Account special price'; ?></span>
                                                    <?php else: ?>
                                                        <span class="text-white font-bold text-[13px]"><?php echo formatCurrency((float) $vData['price']); ?></span>
                                                    <?php endif; ?>
                                                </span>
                                            </span>
                                            <?php if ($isRemote): ?>
                    ...
                    <nav class="mt-5 flex flex-wrap items-center justify-center gap-2" aria-label="Store pages">
                        <?php if ($storePage > 1): ?>
                            <a href="<?php echo htmlspecialchars($buildStoreUrl(['page' => $storePage - 1]), ENT_QUOTES, 'UTF-8'); ?>"
                               class="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-gray-200 hover:bg-blue-500/20">
                        ...
                        ?>
                            <a href="<?php echo htmlspecialchars($buildStoreUrl(['page' => $pageNumber]), ENT_QUOTES, 'UTF-8'); ?>"
                               class="min-w-9 px-3 py-2 rounded-lg border text-center text-sm <?php echo $pageNumber === $storePage ? 'bg-blue-500 text-white border-white/20' : 'bg-white/5 text-gray-300 border-white/10 hover:bg-blue-500/20'; ?>">
                                <?php echo $pageNumber; ?>
                            </a>
                        <?php endfor; ?>
    ...
    <div class="overlay" id="overlay" onclick="closeQuantityModal()" style="display: none;" aria-hidden="true"></div>

    <div class="variant-modal bg-[#1b2336] rounded-[16px] overflow-hidden animate-slide-up shadow-2xl"
        id="quantityModal" style="max-width: 340px; display: none;" role="dialog" aria-modal="true" aria-hidden="true" tabindex="-1">
        ...
        <div class="p-5 pt-3">
            ...
            <div class="flex gap-3">
                ...
                <button type="button" onclick="closeQuantityModal()"
                    class="flex-1 bg-[#242b45] hover:bg-[#2a3454] text-[#8e9bb0] hover:text-white py-2.5 rounded-[10px] font-medium transition-colors text-[14px]" data-lang="buy.cancel">
                    <?php echo Lang::t('buy.cancel'); ?>
                </button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function updateTotalAmount() {
            ...
            // Use 0 if negative for display, but confirmPurchase will handle the error
            ...
            requestInventory(true, 'storefront_refresh').then(function (result) {
                ...
                if (response && response.ok && data && data.success) {
                    if (data.catalog_changed === true) {
                        // If the user already clicked another category/page, the
                        // destination request will render fresh catalogue data. Do
                        // not let this stale background response reload the old URL.
                        if (buyNavigationPending()) return;
                        beginBuyNavigation();
            ...
            try {
                ...
                if (response.ok && data && data.success) {
                    if (data.catalog_changed === true) {
                        ...
                        // page rendered. Reload once unless the user is already
                        // navigating away; the destination page will be fresh.
                        if (buyNavigationPending()) return;
                        beginBuyNavigation();
                        window.location.reload();
                        return;
