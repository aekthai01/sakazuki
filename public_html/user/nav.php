...
$branding = function_exists('getStoreBranding') ? getStoreBranding() : ['title_text' => 'STORE', 'title_color' => '#60a5fa', 'title_style' => 'font-extrabold'];
$titleText = $branding['title_text'] ?? 'STORE';
$titleColor = $branding['title_color'] ?? '#60a5fa';
$titleStyle = $branding['title_style'] ?? 'font-extrabold';
$userTitleIconAsset = function_exists('getTitleIconAsset') ? getTitleIconAsset('user') : ['path' => '', 'version' => 0];
$userTitleIconPath = (string) ($userTitleIconAsset['path'] ?? '');
$userTitleIconUrl = $userTitleIconPath !== ''
  ? '../' . ltrim($userTitleIconPath, '/') . ((int) ($userTitleIconAsset['version'] ?? 0) > 0 ? '?v=' . (int) $userTitleIconAsset['version'] : '')
  : '';
...
.loader-spinner{width:3.5rem;height:3.5rem;border:3px solid rgba(96,165,250,0.1);border-top-color:#60a5fa;border-radius:50%;animation:premium-spin 0.8s cubic-bezier(0.4,0,0.2,1) infinite;filter:drop-shadow(0 0 8px rgba(96,165,250,0.4))}
.loader-text{margin-top:1.5rem;color:#60a5fa;font-size:0.875rem;font-weight:600;letter-spacing:0.2em;text-transform:uppercase;animation:premium-pulse 1.5s ease-in-out infinite;text-shadow:0 0 10px rgba(96,165,250,0.3)}
...
<div class="glass sticky top-0 z-50 border-b border-white/10">
  <div class="nav-topbar-row flex justify-between items-center gap-2 px-3 md:px-6 py-3 md:py-4">
    ...
    <div class="nav-primary-cluster flex items-center gap-2 md:gap-4">
      <button type="button" class="2xl:hidden shrink-0 text-blue-400 focus:outline-none" data-nav-drawer-toggle aria-controls="drawer" aria-expanded="false" onclick="return openDrawer(event)">
        <i class="bi bi-list text-2xl"></i>
      </button>

      <a href="dashboard.php"
        class="brand-home text-xl md:text-2xl <?php echo htmlspecialchars($titleStyle); ?> hover:opacity-90 transition flex items-center px-3 py-1.5 rounded-lg hover:bg-blue-400/10"
        style="color: <?php echo htmlspecialchars($titleColor); ?>;">
        <?php if ($userTitleIconUrl !== ''): ?>
          <img src="<?php echo htmlspecialchars($userTitleIconUrl, ENT_QUOTES, 'UTF-8'); ?>"
               alt=""
               width="32"
               height="32"
               ...
               onerror="this.remove()">
        <?php endif; ?>
        <span class="brand-title-text"><?php echo htmlspecialchars($titleText); ?></span>
      </a>

      <nav class="hidden 2xl:flex min-w-0 items-center space-x-1">
        <a href="dashboard.php" class="px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition flex items-center" data-lang="nav.dashboard"><i class="bi bi-speedometer2 mr-2"></i><?php echo Lang::t('nav.dashboard'); ?></a>
        <a href="buy.php" class="px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition flex items-center" data-lang="nav.buy"><i class="bi bi-bag-check mr-2"></i><?php echo Lang::t('nav.buy'); ?></a>
        <a href="mykeys.php" class="px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition flex items-center" data-lang="nav.my_keys"><i class="bi bi-key mr-2"></i><?php echo Lang::t('nav.my_keys'); ?></a>
        
        <!-- More Dropdown -->
        <div class="relative" data-nav-dropdown-root>
          <button type="button" data-nav-dropdown-toggle="userDropdown" aria-controls="userDropdown" aria-expanded="false" class="px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition flex items-center">
          ...
          <div id="userDropdown" data-nav-dropdown-menu aria-hidden="true" class="nav-dropdown-menu absolute top-full left-0 mt-1 w-52 glass border border-white/10 rounded-lg shadow-xl hidden z-50">
            <a href="deposit.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition flex items-center" data-lang="nav.deposit"><i class="bi bi-wallet2 mr-2"></i><?php echo Lang::t('nav.deposit'); ?></a>
            <a href="history.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition flex items-center" data-lang="nav.history"><i class="bi bi-clock-history mr-2"></i><?php echo Lang::t('nav.history'); ?></a>
            <a href="rankings.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition flex items-center" data-lang="nav.rankings"><i class="bi bi-trophy mr-2"></i><?php echo Lang::t('nav.rankings'); ?></a>
            <a href="reseller_program.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition flex items-center"><i class="bi bi-shop-window mr-2"></i><?php echo $currentLang === 'en' ? 'Become a Reseller' : 'สมัครตัวแทน'; ?></a>
            <a href="account.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition flex items-center" data-lang="nav.account"><i class="bi bi-person-gear mr-2"></i><?php echo Lang::t('nav.account'); ?></a>
          </div>
        </div>
      </nav>
    ...
    <div class="nav-actions flex items-center gap-1.5 md:gap-2">
      <div class="hidden 2xl:flex items-center gap-2">
        <span class="text-gray-400 text-sm md:text-base"><?php echo htmlspecialchars($_SESSION['username']); ?></span>
        <?php
        $roleClass='bg-blue-500/10 text-blue-400 border-blue-500/20';
        if(isAdmin()) $roleClass='bg-red-500/10 text-red-400 border-red-500/20';
        elseif(isReseller()) $roleClass='bg-purple-500/10 text-purple-400 border-purple-500/20';
        ?>
        <span class="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase border <?php echo $roleClass; ?>">
...
<aside id="drawer" class="drawer glass border-r border-white/10 flex flex-col" aria-hidden="true" aria-label="User navigation">
  <div class="p-4 border-b border-white/10 flex items-center justify-between">
    <div class="flex items-center gap-2 min-w-0">
      <?php if ($userTitleIconUrl !== ''): ?>
        <img src="<?php echo htmlspecialchars($userTitleIconUrl, ENT_QUOTES, 'UTF-8'); ?>" alt="" width="24" height="24" class="w-6 h-6 max-w-6 max-h-6 object-contain shrink-0 rounded bg-white/5 border border-white/10" onerror="this.remove()">
      <?php else: ?>
        <i class="bi bi-cart text-blue-400 shrink-0"></i>
      <?php endif; ?>
      <span class="font-bold truncate" style="color:<?php echo htmlspecialchars($titleColor); ?>;"><?php echo htmlspecialchars($titleText); ?></span>
    </div>
    <button type="button" class="text-gray-300 hover:text-white" onclick="return closeDrawer(event)" aria-label="Close menu"><i class="bi bi-x-lg"></i></button>
  </div>

  <nav class="nav-drawer-scroll p-3 space-y-1 flex-1">
    <a href="dashboard.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition" data-lang="nav.dashboard"><i class="bi bi-speedometer2 mr-2"></i><?php echo Lang::t('nav.dashboard'); ?></a>
    <a href="deposit.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition" data-lang="nav.deposit"><i class="bi bi-wallet2 mr-2"></i><?php echo Lang::t('nav.deposit'); ?></a>
    <a href="buy.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition" data-lang="nav.buy"><i class="bi bi-bag-check mr-2"></i><?php echo Lang::t('nav.buy'); ?></a>
    <a href="mykeys.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition" data-lang="nav.my_keys"><i class="bi bi-key mr-2"></i><?php echo Lang::t('nav.my_keys'); ?></a>
    <a href="history.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition" data-lang="nav.history"><i class="bi bi-clock-history mr-2"></i><?php echo Lang::t('nav.history'); ?></a>
    <a href="rankings.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition" data-lang="nav.rankings"><i class="bi bi-trophy mr-2"></i><?php echo Lang::t('nav.rankings'); ?></a>
    <a href="reseller_program.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition"><i class="bi bi-shop-window mr-2"></i><?php echo $currentLang === 'en' ? 'Become a Reseller' : 'สมัครตัวแทน'; ?></a>
    <a href="account.php" class="block px-4 py-2 rounded-lg hover:bg-blue-400/10 hover:text-blue-400 transition" data-lang="nav.account"><i class="bi bi-person-gear mr-2"></i><?php echo Lang::t('nav.account'); ?></a>
  </nav>
