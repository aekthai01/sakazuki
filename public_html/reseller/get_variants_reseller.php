...
if (!ensureResellerVariantPricesTable()) {
    ...
    echo '<div class="text-center py-6 text-gray-400">Unable to load pricing</div>';
    exit;
}
...
$grouped = cgoGetUnifiedStoreVariants((int) $productId, 'reseller', (int) ($_SESSION['user_id'] ?? 0));
...
if (!$grouped): ?>
    <div class="text-center py-6 text-gray-400">
        ...
        <button type="button" onclick="closeVariantModal()" class="px-3 py-1.5 bg-green-500 hover:bg-green-600 text-white rounded text-sm transition">Close</button>
    </div>
<?php else: ?>
    <div class="space-y-2">
        ...
        ?>
            <button type="button"
                    ...
                    class="w-full text-left glass border border-white/10 rounded p-3 hover:border-green-500/50 hover:bg-green-500/5 transition flex justify-between items-center group">
                <div class="flex-grow">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-green-400 font-bold text-sm"><?php echo formatCurrency($price); ?></span>
                    <i class="bi bi-chevron-right text-gray-400 group-hover:text-green-400 transition"></i>
                </div>
            </button>
        <?php endforeach; ?>
    </div>
